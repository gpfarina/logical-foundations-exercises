...
(** Use [filter] (instead of [Fixpoint]) to write a Coq function
    ...
    7. *)
Search blt_nat.
Definition filter_even_gt7 (l : list nat) : list nat:=  filter (fun n=> andb (evenb n) (blt_nat 7 n)) l.
...
(** Show that [map] and [rev] commute.  You may need to define an
    auxiliary lemma. *)
Theorem map_aux: forall (X Y: Type) (f: X -> Y) (l1 l2: list X),
...
Proof.
  ...
  induction l.
  - reflexivity.
  - simpl. rewrite map_aux.  rewrite IHl.
    reflexivity.
Qed.
...
Definition mult (n m : nat) : nat.  Admitted.
