...
Proof.
  ...
  - (* n = S n' *) intros eq. destruct m as [| m'].
    + (* m = O *) inversion eq.
    + (* m = S m' *) inversion eq. apply f_equal with (f:= double )in H0.
...
Theorem f_equal1 : forall (A B : Type) (f: A -> B) (x y: A),
  x = y -> f x = f y.
Proof. intros A B f x y eq. rewrite eq.  reflexivity.  Qed.
...
Theorem split_combine : forall (X: Type) (Y: Type) (l: list (X * Y)) (l1 : list X) (l2: list Y) ,
    combine l1 l2 = l -> length l1 = length l2 -> split l = (l1, l2).
  Proof.
intros X Y l.
induction l.
...
- intros. 
  ...
  + destruct l2.
    * inversion H0. 
    * simpl in H. inversion H. rewrite H3.
      apply IHl in H3.
      simpl. 
      rewrite H3.
      reflexivity.
