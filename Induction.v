...
Theorem. addition is commutative, that is for all naturals m n, n+m=m+n
by induction on n.
case n=0.
   We have to show that 0 + m = m + 0.
   starting from m=m, using definitoin of plus and the fact that 0 is the right neuatral element 
   for + we get that 0 + m = m + 0.

   Case n=n'+1.
   We have to show that (n'+1) + m = m + (n'+1) assuming that
   H. forall t n'+t=t+n'.
   So from H we know that: n'+m=m+n'.
   Since forall x, x+q +1 = x + (q + 1)
   when x=m and q=n' we get that m+(n'+1) = n'+m+1 which by 
   commutaivviyt of equaility gives us n'+m+1=m+(n'+1) which is what we need.
    Proof: (* FILL IN HERE *)
*)
(** [] *)
...
Theorem lemma_mult: forall m n: nat,
       n + n * m = n * S m.
Proof.
intros.
induction n.
